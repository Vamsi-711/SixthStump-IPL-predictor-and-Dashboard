...
import re
# Match the entire block safely. using re.DOTALL so .* matches newlines.
html = re.sub(r'const POINTS_TABLE_2026 = \[\s*\{.*?\n\s*\];', new_points, html, flags=re.DOTALL)
html = re.sub(r'const ISTAT2026 = \{\s*orange:.*?\n\s*\};', new_istat, html, flags=re.DOTALL)

with open('public/index.html', 'w', encoding='utf-8') as f:
    f.write(html)

print("index.html actually patched!")
